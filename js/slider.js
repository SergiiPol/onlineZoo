...
// make new array with sorted items through step (6 or 4 )
    ...
    for (let i = 0; i < newCardsArray.length; i++) {
        if (i === 0) {
            newCardsArray[0] = shuffle(newCardsArray[0])
            ...
            newCardsArray[i] = shuffle(newCardsArray[i])
...
function makeItemsArray2() {

let boxes = document.querySelectorAll('.shuffle')
    boxes.forEach(el =>{
        let mixedArray = []
        let boxesItems = el.querySelectorAll('.animal-card')
        boxesItems.forEach(elem =>{mixedArray.push(elem.outerHTML)})
        mixedArray = shuffle(mixedArray)
        el.innerHTML = ''
        mixedArray.forEach(elem =>{
            el.innerHTML +=elem
        })
    })
...
// slider for animal cards
    ...
    function hideItem(direction) {
        ...
        cards[currentItem].classList.add('shuffle');
        cards[currentItem].addEventListener('animationend', function () {
           this.classList.remove('active', direction);
            makeItemsArray2()

            setTimeout(()=>{this.classList.remove('shuffle');},100)
